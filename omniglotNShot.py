...
class OmniglotNShot:
    ...
    def getHoleTrain(self):

        data_pack = self.datasets['train']

        task_number = data_pack.shape[0] // self.n_way # 1200 / (5) = 240
        idxs = np.array(range(data_pack.shape[0]))
        ...
        x_spts, y_spts, x_qrys, y_qrys = [], [], [], []
        for i in range(task_number):  # one batch means one set

            x_spt, y_spt, x_qry, y_qry = [], [], [], []
            selected_cls = np.array(idxs[i*self.n_way:(i+1)*self.n_way])
        ...
        # [b, setsz, 1, 84, 84]
        x_spts = np.array(x_spts).astype(np.float32).reshape(task_number, setsz, 1, self.resize, self.resize)
        y_spts = np.array(y_spts).astype(np.int).reshape(task_number, setsz)
        # [b, qrysz, 1, 84, 84]
        x_qrys = np.array(x_qrys).astype(np.float32).reshape(task_number, querysz, 1, self.resize, self.resize)
        y_qrys = np.array(y_qrys).astype(np.int).reshape(task_number, querysz)
